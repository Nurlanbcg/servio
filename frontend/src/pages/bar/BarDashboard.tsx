...
const BarDashboard: React.FC = () => {
    ...
    const fetchOrders = async () => {
        try {
            const { data } = await api.get('/bar/orders');
            setOrders(data);
        } catch {
    ...
    const markItemDone = async (orderId: string, itemIndex: number) => {
        try {
            await api.patch(`/bar/orders/${orderId}/items/${itemIndex}`);
            setOrders((prev) =>
                prev
                    .map((o) =>
                        o._id === orderId
    ...
    return (
        <Layout title="Bar Display">
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        <h2 className="text-2xl font-bold text-surface-100">Bar Orders</h2>
                ...
                {loading ? (
                    ...
                    <div className="card text-center py-20">
                        ...
                        <p className="text-surface-500 text-sm mt-1">New drink orders will appear here automatically</p>
                    </div>
                ) : (
